...
echo -e "${BLUE}🔍 Testing Akka Cache Endpoints (JSON API)${NC}"
echo -e "${BLUE}==========================================${NC}"
...
# 4. Cache operations (JSON format)
test_endpoint "PUT" "/cache/test-key" '{"value":"test-data"}' "application/json" "200" "Cache PUT operation (JSON)"
test_endpoint "GET" "/cache/test-key" "" "" "200" "Cache GET operation (returns JSON)"
...
echo -e "  PUT  $BASE_URL/cache/{key}      - Store value (JSON format)"
echo -e "  GET  $BASE_URL/cache/{key}      - Get value (returns JSON)"
echo -e "  DELETE $BASE_URL/cache/{key}    - Delete value"
echo

echo -e "${YELLOW}💡 Correct JSON API usage:${NC}"
echo -e "  curl $BASE_URL/"
echo -e "  curl $BASE_URL/admin/status"
echo -e "  curl -X PUT $BASE_URL/cache/hello \\\\"
echo -e "       -H 'Content-Type: application/json' \\\\"
echo -e "       -d '{\"value\":\"world\"}'"
